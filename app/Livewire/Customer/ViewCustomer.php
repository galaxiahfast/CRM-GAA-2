...
class ViewCustomer extends IndexCustomer
{
    ...
    public function mount($customer = null)
    {
        ...
        $this->services = $this->customer->services->pluck('service_id') ?? null;
    ...
    protected function calculateTotalPdfRequired()
    {
        ...
        // dump('contador inicial: ' . $this->countPDFs);

            // Validar sub_servicio id 1 (states)
            if($servicesIds->contains(1)) {
                $totalRequired += $this->calculateComplementaryFiles($this->customer->states);
                dump('total después de 1: ' . $totalRequired);
            }
            
            // Validar sub_servicio id 6 (statements)
            if ($servicesIds->contains(6)) {
                $totalRequired += $this->calculateComplementaryFiles($this->customer->statements);
                dump('total después de 6: ' . $totalRequired);
            }

        $otherServices = $this->customer->services->whereNotIn('id', [1, 6]);
        if ($otherServices->isNotEmpty()) {
            dump($otherServices);
            $totalRequired += $this->calculateComplementaryFiles($otherServices);
        }

        dump('total final: ' . $totalRequired);
        dump($this->complementaryFiles);
        return $totalRequired;
    }

    private function calculateComplementaryFiles($services)
    {
        $complementaryTotal = 0;
        foreach ($services as $service) {
            $complementaryTotal += 2; // Acuse y Comprobante
            $isComplementariaAcuse = $this->pdfsDB['complementaria'][$service->id]['acuse'] ?? collect();
            // $isNormalComprobante = $this->pdfsDB['normal'][$service->id]['comprobante'] ?? collect();
            
            if(!isset($this->complementaryFiles[$service->id])){
                $this->complementaryFiles[$service->id] = [
                    'acuser' => [],
                    'comprobante' => [],
                ];
            }

            if ($isComplementariaAcuse->isNotEmpty()) {
                foreach ($isComplementariaAcuse as $file) {
                    $this->complementaryFiles[$service->id]['comprobante'][] = [
                        'id' => $file->id ?? uniqid(),
                        'folioe' => $file->id,
                    ];
                }
        ...
        dump("Intermedio doble ciclo: " . $complementaryTotal);
        
        foreach ($this->complementaryFiles as $files) {
            if (!empty($files['acuse'])) {
                $complementaryTotal += 2;
            }
        }
    ...
    private function handlePdfUpload($declarationType, $fileType)
    {
        ...
        $this->validate([
            "pdfs.$declarationType.$fileType" => 'required|file|mimes:pdf|max:2048',
        ]);

        $pdfFile = $this->pdfs[$declarationType][$fileType];

        if(!$pdfFile) {
            $this->addError("pdfs.$declarationType.$fileType", 'No se encontró el archivo a sabir');
            return;
        }
                ...
                if($this->selectedSubServiceId == 1) {
                    foreach ($this->states as $state) {
                        ...
                        if($isComplementariaAcuse->isEmpty()) {
                            dump("No se puedo mitelvi", $isComplementariaAcuse);
